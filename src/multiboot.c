...
extern const uint8_t rom_start[] asm("_binary_test_gba_start");
extern const uint8_t rom_end[] asm("_binary_test_gba_end");
...
void multiboot() {
    ...
    long romLength = (uint32_t)rom_end - (uint32_t)rom_start;
    ...
    while (fcnt < rom_size) {
        ...
        w = rom_start[i] | (rom_start[i + 1] << 8) | (rom_start[i + 2] << 16) | (rom_start[i + 3] << 24);
